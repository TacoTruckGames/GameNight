...
migrations and re-seeds the demo board every time it starts (seed dates are relative to _now_, so the "FULL"
...
pnpm typecheck
pnpm lint                        # ESLint (type-aware) + Prettier --check; `pnpm format` writes
```
...
shared/          zod schemas + API types + maps links + game types, imported by both worker and client
src/             React client — pages/, components/, api/ (query hooks), identity/ (picker + context),
                 admin/ (the operator site, its own hooks), lib/, theme/ (tokens.css + base.css)
migrations/      D1 schema        seed/   demo board (idempotent SQL)
test/            vitest, runs inside workerd against real local D1 + DOs
scripts/         db-reset-local, deploy, stress (the seat race), populate (to launch scale), loadtest
tools/           contrast-audit (both themes, every token pair), artgen (the icon set's generator)
...
| `GET /api/me` (who am I), `GET /api/health` (liveness, no database)                                               | ✓      | ✓                                     |
| `GET /api/places/config`, `GET /api/events/:id/map` (public); `GET /api/places/suggest`, `GET /api/places/map`    | —      | ✓ (and admin) — see "Venues and maps" |
| `/api/admin/*` — 15 routes, every one behind `requireAdmin`; listed at the top of `worker/routes/admin.ts`        | 403    | 403                                   |
...
a demo board to get into, from a phone, without seeding a database first. The part that _is_ load-bearing is
...
1. **The Durable Object is the linearization point.** All RSVP/cancel calls for one event go through one
   ...
   middle of a mutation, and Durable Object input gates only protect _storage_ awaits — without it two
...
- **Sort** (`?sort=`) is `date` (soonest first, the default) or `popular`: fullest-first by _ratio_ of seats
...
- **Agenda headers.** The board groups cards by _local_ day ("Fri, Sep 18 · 3 events") in all three views.
...
- **Week / Month / List** are three readings of one `GET /api/events` — no second endpoint, no second
  ...
  Both dated views have to fill the days _behind_ today — a grid you cannot page backwards through is a
  ...
  two ends are computed in _local_ time and sent as UTC, because the cells bucket by local day. Monday-first
  ...
  month grid and the week strip become navigators on the _right_, with the cards beside them in the left
  ...
  the list keeps its days in one chronological column and pairs the cards _within_ a day two across. The
...
- **The card is 88px, and was 223px.** Measured at 390×844: 66px of the old card was padding and gaps, and
  ...
  and venue — and one placement does the work: the **title spans to the right edge** with the action _below_
...
- **The detail is a sheet over the board**, not a page you navigate to. Tapping a card passes
  `state.backgroundLocation`, so `routes.tsx` renders the board _and_ the event on top of it; a typed URL, a
  ...
  distance _or_ speed, because refusing a short flick is what makes a sheet feel stuck. The long date also stopped sharing a line with the attendance count, which is what made
...
- **An organizer edits their own event** from its page — the same `EventForm` that posts one, prefilled, and
  ...
  deliberately _not_ audited — `audit()` writes the trail the dashboard reads as "recent admin actions", and
...
- **Editing replaces the view.** Edit used to append the form under the read view, so the sheet became the
  event, then the form, then the guest list. Now the form _is_ the view: the header and the two facts — head
...
what I typed", or pressing Remove drops it. There is no venue _name_ column because `displayName` is a
...
The live demo has it configured; a fresh clone does not, and that is a supported state, described next.
...
covered: the picker's "use what I typed" row posts a free-text venue, and the client renders it (checked
by a Playwright pass, ad hoc — not in the repo).
...
mechanism: the tools **sign themselves in** as the provisioned operator account (_Site Admin_), because the
...
can be open in one tab as _Site Admin_ while the board is open in another as Alice, and neither moves the
...
identical for every user _by design_ — `EventSummary` carries no `myRsvp`; whether you are going is a
...
`pnpm test` runs 424 tests _inside_ the Workers runtime (`@cloudflare/vitest-plugin`) against a real local
...
- every commit is one concern with the _why_ in its message, so `git log` reads as the design record.
...
- **Venues** — because an address a phone can navigate to is the difference between a listing and an
  ...
  _search_ for the same words, which degrades honestly — a search that finds nothing shows you that. **The form shows the map too**,
  ...
  other map here — a _search_ rather than directions, because at that point the organizer is checking they
